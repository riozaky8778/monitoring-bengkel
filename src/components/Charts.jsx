  ...
  useEffect(() => {
    ...
    chartRef.current = new Chart(ref.current, {
      ...
      data: {
        ...
        datasets: [{ data: data.map(d => d.count), backgroundColor: data.map((_, i) => COLORS[i % COLORS.length]), borderWidth: 0, hoverOffset: 6 }]
      },
      options: { cutout: '72%', responsive: false, plugins: { legend: { display: false } }, animation: { duration: 800 } }
    });
        ...
        {data.map(({ depo, count }, i) => (
          <div key={depo} className="donut-legend-item">
            ...
            <span className="donut-legend-val">
              {count}
              <span style={{ fontSize:10, color:'var(--text3)', marginLeft:5 }}>({total > 0 ? Math.round(count/total*100) : 0}%)</span>
            </span>
          </div>
        ))}
      </div>
